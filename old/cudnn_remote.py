...
n_classes = 2
n_layers = 1
sn = math.sqrt(1)/math.sqrt(2*n_input)#+n_hidden)#+n_hidden)#+n_hidden) #Glorot initialisation, var(p(x))
forget_gate_init = 5.0                          # = 1/(n_in). We use uniform p(x)
clip = 1000 #We use gradient clipping to stop the gradient exploding initially
...
for n in range(2):
    weight_list.append(np.float32(
        np.random.uniform(low=-math.sqrt(3.0/2)*sn, high=math.sqrt(3.0/2)*sn,
                          size=[n_hidden, n_input])))
for n in range(2,3):
    weight_list.append(np.float32(
        np.random.uniform(low=-math.sqrt(6)*sn, high=math.sqrt(6)*sn,
                          size=[n_hidden, n_input])))
for n in range(3,4):
    weight_list.append(np.float32(
        np.random.uniform(low=-math.sqrt(3.0/2)*sn, high=math.sqrt(3.0/2)*sn,
...
with tf.device("gpu:0"):
    with tf.Session() as sess:
        ...
        while step * batch_size < training_iters:
            batch_x, batch_y = gen_2b_data(n_steps-1, n_input-1, batch_size, num)
