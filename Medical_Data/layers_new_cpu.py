...
lr_mod = tf.load_op_library('./LinearRecurrenceNew.so')
print dir(lr_mod)
...
def linear_surrogate_lstm(X, hidden_size, name='lin_sur_lstm'):
    with vscope(name):
        ...
        c = lr_mod.linear_recurrence_new(f, i * z)
        h = o * c
        return h
...
def gilr_layer(X, hidden_size, nonlin=tf.nn.elu,
    ...
    with vscope(name):
        ...
        return lr_mod.linear_recurrence_new(gate, (1-gate) * impulse)

# def LinearRecurrence(f, b):
...
#         hs.append(to_append)
#         if index % 1000 == 0:
#             print 'Up to index {}'.format(index)
#     return tf.stack(hs)
